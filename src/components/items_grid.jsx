...
export function ItemsGrid({ currentPage }) {
  ...
  return (
    <>
      ...
      <div className='flex flex-col items-center'>
        <div className='grid grid-cols-4 gap-4 mt-4 p-10 w-full max-w-7xl'>
          {currentPosts.map(product => (
            <div key={product.id} className='border p-4 rounded w-full max-w-sm bg-white shadow transition-transform duration-200 hover:scale-105 cursor-pointer'>
              ...
              <button className='hover:cursor-pointer active:bg-lime-800 flex justify-center items-center w-full max-w-md p-1 rounded-md bg-lime-500'>
